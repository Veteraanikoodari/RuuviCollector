...
class PersistenceServiceTest {
  ...
  void testApplyingDifferentLimitingStrategiesToDifferentDevices() {
    ...
    service.store(
        withAcceleration(withRssi(handler.handle(hciData).get(), 1), 1d)); // Store: first instance
    service.store(
        withAcceleration(withRssi(handler.handle(hciData).get(), 2), 1d)); // Not: no time passed
    setClockToMilliseconds(1000);
    service.store(
        withAcceleration(
            withRssi(handler.handle(hciData).get(), 3), 1d)); // Not: still not enough time passed
    setClockToMilliseconds(3000);
    service.store(
        withAcceleration(withRssi(handler.handle(hciData).get(), 4), 1d)); // Not: same as above
    service.store(
        withAcceleration(withRssi(handler.handle(hciData2).get(), 5), 1d)); // Store: first instance
    ...
    service.store(
        withAcceleration(
            withRssi(handler.handle(hciData2).get(), 7), 1.99d)); // Store: movement detected
    setClockToMilliseconds(10000);
    service.store(
        withAcceleration(
            withRssi(handler.handle(hciData).get(), 8), 1d)); // Store: enough time has passed
    service.store(
        withAcceleration(
            withRssi(handler.handle(hciData2).get(), 9), 1d)); // Store: movement detected
    setClockToMilliseconds(11000);
    service.store(
        withAcceleration(
            withRssi(handler.handle(hciData).get(), 10), 1d)); // Not: not enough time passed
    service.store(
        withAcceleration(
    ...
    service.store(
        withAcceleration(
            withRssi(handler.handle(hciData2).get(), 13), 1d)); // Store: enough time passed
