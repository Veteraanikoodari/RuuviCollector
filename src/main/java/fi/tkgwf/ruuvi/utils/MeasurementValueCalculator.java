...
public class MeasurementValueCalculator {
  ...
  /**
   * Calculates values that can be calculated based on other values, such as total acceleration and
   * absolute humidity
   *
  ...
  public static EnhancedRuuviMeasurement calculateAllValues(EnhancedRuuviMeasurement measurement) {
    ...
    measurement.setAirDensity(
        airDensity(
            measurement.getTemperature(), measurement.getHumidity(), measurement.getPressure()));
  ...
  public static Double airDensity(Double temperature, Double relativeHumidity, Double pressure) {
    ...
    return 1.2929
        * 273.15
        / (temperature + 273.15)
        * (pressure - 0.3783 * relativeHumidity / 100 * equilibriumVaporPressure(temperature))
        / 101300;
  }
}
