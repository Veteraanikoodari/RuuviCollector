 ...
 *
 * <p>The time limit is defined as {@link Config#getMeasurementUpdateLimit()}. The acceleration
 * bounds are defined as values compared to the previous measurement with {@link
 * Config#getDefaultWithMotionSensitivityStrategyThreshold()}.
 *
...
public class DefaultDiscardingWithMotionSensitivityStrategy implements LimitingStrategy {
    ...
    private final Map<String, List<EnhancedRuuviMeasurement>> previousMeasurementsPerMac =
            new HashMap<>();
    ...
    public Optional<EnhancedRuuviMeasurement> apply(final EnhancedRuuviMeasurement measurement) {
        final List<EnhancedRuuviMeasurement> previousMeasurements =
                previousMeasurementsPerMac.getOrDefault(measurement.getMac(), new LinkedList<>());
        previousMeasurements.add(measurement);
        if (previousMeasurements.size()
                > Config
                        .getDefaultWithMotionSensitivityStrategyNumberOfPreviousMeasurementsToKeep()) {
